...
if [ ${stage} -le 3 ] && [ ${stop_stage} -ge 3 ]; then
  ...
  if ${use_gan_loss}; then
    train_script=wesep2/bin/train_gan.py  # Will remove
  else
    train_script=wesep2/bin/train.py
  fi
...
if [ ${stage} -le 4 ] && [ ${stop_stage} -ge 4 ]; then
  ...
  python wesep2/bin/average_model.py \
    --dst_model $avg_model \
...
if [ ${stage} -le 5 ] && [ ${stop_stage} -ge 5 ]; then
  ...
  python wesep2/bin/infer.py --config $config \
    --fs ${fs} \
    --gpus 0 \
    --exp_dir ${exp_dir} \
    --data_type "${data_type}" \
